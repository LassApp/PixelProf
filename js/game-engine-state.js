...
function _resumeAfterDialog(){
  if(gsIs(GS.PAUSED)){
    ...
    // ── Riprende i timer via manager (solo quelli che non erano già paused) ──
    // TimerManager usa pausedByDialog per sapere quali riprendere.
    // Per Speed Quiz il fnResume del manager chiama _restartSpeedTimer(),
    // ma la UI va ripristinata qui PRIMA della ripresa del timer.
    ...
    // ── UI per Memory: sempre rilascia il lock; overlay solo se non era già paused ──
    if(gameType==='memory'){
    ...
    // ── UI per Abbina: sempre rilascia il lock; overlay solo se non era già paused ──
    if(gameType==='match'){
      _setGamePauseLock(false);
      const mbtn=sh('match-pause-btn');
      const micon=sh('match-pause-icon');
    ...
    // ── Ripristina timer via manager (DOPO aggiornamento UI) ──
